...
function getlinkPoster(data) {
    ...
    data.map(function(val)  {
        var start = val.image.indexOf("http://phimmoi");
        ...
        var data = {
            ...
            "titlev": val.titlev,
            "titlew": val.titlew,
            "image": val.image.substring(start, end + 16).replace('http','https'),
            "time": val.time
        }
        newArray.push(data);
    })
...
function getlinkPosterDetails(data) {
    ...
    var data2 = {
        ...
        "image": data.image.substring(start, end + 17).replace('http','https'),
...
function getlinkPosterActor(data) {
    var newArray = [];
    data.map(function(val)  {
        var start = val.img.indexOf("http://image.phimmoi.");
        var end = val.img.indexOf("/thumb.jpg");
        var data = {
            "id": val.id,
            "url": val.url,
            "img": val.img.substring(start+23, end + 10).replace('http','https'),
            "name": val.name,
